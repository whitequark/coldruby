...
var EventEmitter = ruby.define_module_under(NodeJS, 'EventEmitter');
...
function addListenerWith(func, self, event, block) {
  ...
  var protector = ruby.protect_node(function() {
    var new_args = [];
    if(handler != null)
      new_args = handler.apply(ruby, arguments);

    ruby.funcall2(block, 'call', new_args);
  });
...
ruby.define_method(EventEmitter, 'on', 1, function(self, event) {
...
ruby.alias_method(EventEmitter, 'add_listener', 'on');

ruby.define_method(EventEmitter, 'once', 1, function(self, event) {
  var event = this.to_str(event).value, block = this.block_proc();
...
ruby.define_method(EventEmitter, 'remove_all_listeners', 1, function(self, event) {
  self.internal.removeAllListeners(this.to_str(event).value);

  return this.builtin.Qnil;
});

ruby.define_method(EventEmitter, 'set_max_listeners', 1, function(self, max) {
  self.internal.setMaxListeners(this.to_int(max));

  return this.builtin.Qnil;
});

ruby.define_method(EventEmitter, 'remove_listener', 2, function(self, event, block) {
  self.internal.removeListener(event, block.wrapper);

  return this.builtin.Qnil;
});

ruby.define_method(EventEmitter, 'listeners', 1, function(self, event) {
